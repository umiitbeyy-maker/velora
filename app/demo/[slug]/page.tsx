  ...
  useEffect(() => {
    fetch('/api/businesses')
      ...
      .then(data => {
        ...
        if (biz) {
          setBusiness(biz)
          setMessages([{ role: 'assistant', content: `Hi! I am the AI assistant for ${biz.name}. How can I help you?` }])
        }
      })
  }, [slug])
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: '#fff', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {messages.map((msg, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
            <div style={{ maxWidth: '70%', padding: '12px 16px', borderRadius: '12px', fontSize: '14px', lineHeight: '1.6',
              background: msg.role === 'user' ? '#a855f7' : '#1a1a1a', color: '#fff' }}>
              {msg.content}
            </div>
          </div>
        ))}
        {loading && (
          <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
            <div style={{ padding: '12px 16px', background: '#1a1a1a', borderRadius: '12px', color: '#888', fontSize: '14px' }}>Thinking...</div>
          </div>
        )}
        <div ref={bottomRef} />
      </div>
      <div style={{ padding: '16px 24px', borderTop: '1px solid #1a1a1a', display: 'flex', gap: '10px' }}>
        ...
        <button onClick={send} disabled={loading || !input.trim()}
          style={{ padding: '12px 20px', background: '#a855f7', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit' }}>
          Send
        </button>
      </div>
    </div>
