...
export default function OnboardingPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.business) {
        router.push(`/demo/${data.business.slug}`)
      }
    } catch (e) {
      console.error(e)
    }
      ...
      <div style={{ width: '100%', maxWidth: '560px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          <div style={{ fontSize: '32px', fontWeight: '700', marginBottom: '8px' }}>
            <span style={{ color: '#a855f7' }}>Velora</span>
          </div>
        ...
        {step === 1 && (
          <div>
            ...
            <button
              ...
              style={{ width: '100%', padding: '14px', background: form.name && form.industry ? '#a855f7' : '#333', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '15px', fontWeight: '600', cursor: form.name && form.industry ? 'pointer' : 'not-allowed' }}>
              Continue →
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            ...
            <button onClick={addService}
              style={{ width: '100%', padding: '10px', background: 'transparent', border: '1px dashed #444', borderRadius: '8px', color: '#888', fontSize: '13px', cursor: 'pointer', marginBottom: '24px' }}>
              + Add service
            </button>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setStep(1)}
                style={{ flex: 1, padding: '14px', background: '#1a1a1a', color: '#ccc', border: '1px solid #333', borderRadius: '8px', fontSize: '15px', cursor: 'pointer' }}>
                ← Back
              </button>
              <button onClick={() => setStep(3)}
                style={{ flex: 2, padding: '14px', background: '#a855f7', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}>
                Continue →
              </button>
            </div>
          </div>
